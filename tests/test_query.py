...
def test_query_filter_labels(datastore):
    ...
    try:
        ...
        assert_equal(10, query(example_query, datastore, start=datetime.now(timezone.utc)-timedelta(hours=1))["eventcount"])
        # Test that endtime works
        assert_equal(10, query(example_query, datastore, end=datetime.now(timezone.utc)-timedelta(hours=1))["eventcount"])
    finally:
        datastore.delete_bucket(bid1)
